...
import ProductDetailModal from "../../shared/Modals/ProductDetailModal";
...
const ProductDetails = ({
  ...
  useEffect(() => {
    ...
    const found = productsData.find(
      (p) => p.name.toLowerCase() === titles.toLowerCase()
    );
  ...
  return (
    <div className="text-black w-full">
      ...
      <div className="bg-gray-200 h-[3rem] flex px-10 items-center w-full">
        <div className="font-black">{`PRODUCTS > ${product.name}`}</div>
      </div>
      <div className="flex flex-col md:flex-row gap-6 justify-center mx-auto items-center px-4 h-[100%] mb-10 md:h-[650px] w-full md:w-[90%]">
        ...
        <div className="flex flex-col items-center md:items-start w-full md:w-1/2 px-4">
          <h1 className="text-3xl md:text-4xl font-black mb-2 text-center md:text-left">
            {product.name}
          </h1>
          ...
          <Button
            ...
            disabled={false}
            className="xl:w-[20%] w-full cursor-pointer"
          />
        </div>
      </div>
